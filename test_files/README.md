...
- `guppy_examples/`
  Guppy source examples written as `.py` [uv script](https://docs.astral.sh/uv/guides/scripts/). Each script defines a test program for a pinned version of
  guppylang. The compiled HUGR is stored alongside it with a `.hugr` extension. The guppylang version used is defined by
  each script.
