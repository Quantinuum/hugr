...
// why Optype require ops, why HugrView doesnot
...
impl CircuitHash for Hugr {
    fn circuit_hash(&self, node: Node) -> Result<u64, HashError> {
        ...
        // Consider change the vistor to DFS
        ...
        // Change this? with not dataflow there is no output node?
        // node_hashes
...
fn hash_node(
    ...
    if hugr.children(node).next().is_some() {
        hugr.circuit_hash(node)?.hash(&mut hasher);
    }
