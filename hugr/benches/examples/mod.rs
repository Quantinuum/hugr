//! Builders and utilities shared by benchmarks.
...
/// The Guppy T-state factory is a representative real-world HUGR.
const T_FACTORY: &[u8] = include_bytes!("../../../test_files/guppy_examples/t_factory.hugr");

/// Loads the bundled Guppy T-state factory.
pub(crate) fn t_factory() -> Hugr {
    Hugr::load(T_FACTORY, None).expect("the bundled T-state factory should be a valid HUGR")
}

/// Builds a minimal DFG that passes its Boolean input through unchanged.
...
/// Adds a three-block conditional control-flow graph to `cfg_builder`.
pub(crate) fn simple_cfg_builder<T: AsMut<Hugr> + AsRef<Hugr>>(
...
/// Builds a small complete control-flow HUGR for builder and insertion benches.
...
/// Builds a DFG that calls a local definition and declaration.
...
/// Quantum operations used by the generated circuit benchmark fixture.
pub(crate) static QUANTUM_EXT: LazyLock<Arc<Extension>> = LazyLock::new(|| {
    Extension::new_arc(
...
/// Standard extensions augmented with the benchmark quantum operations.
pub(crate) static BENCH_EXTENSIONS: LazyLock<ExtensionRegistry> = LazyLock::new(|| {
...
pub(crate) struct CircuitLayer {
    /// The Hadamard node in this layer.
    pub(crate) h: Node,
    /// The first controlled-not node in this layer.
    pub(crate) cx1: Node,
    /// The second controlled-not node in this layer.
    pub(crate) cx2: Node,
}

/// Construct a quantum circuit with two qubits and `layers` layers applying `H q0; CX q0, q1; CX q1, q0`.
pub(crate) fn circuit(layers: usize) -> (Hugr, Vec<CircuitLayer>) {
...
pub(crate) fn big_hugr(byte_size: usize) -> Hugr {
