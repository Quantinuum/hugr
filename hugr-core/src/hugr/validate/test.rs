...
fn typevars_declared() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let f = FunctionBuilder::new(
        ...
        PolyFuncType::new(
            [TypeBound::Linear.into()],
            Signature::new_endo([Type::new_var_use(1, TypeBound::Linear)]),
        ),
    )?;
    let [w] = f.input_wires_arr();
    ...
    let f = FunctionBuilder::new(
        ...
        PolyFuncType::new(
            [TypeBound::Linear.into()],
            Signature::new_endo([Type::new_var_use(1, TypeBound::Copyable)]),
        ),
    )?;
    let [w] = f.input_wires_arr();
    assert!(f.finish_hugr_with_outputs([w]).is_err());
...
fn instantiate_row_variables() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut dfb = DFGBuilder::new(inout_sig(
        vec![
            Type::new_function(Signature::new([usize_t()], vec![usize_t(); 2])),
            usize_t(),
...
fn row_variables() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let inner_ft = Type::new_function(FuncValueType::new_endo(TypeRowRV::new(tv.clone())));
    let ft_usz = Type::new_function(FuncValueType::new_endo(
        TypeRowRV::new(tv.clone()).concat([usize_t()]),
    ));
