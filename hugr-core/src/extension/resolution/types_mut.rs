...
fn resolve_typerow_rv_exts(
    ...
    *row = TypeRowRV::try_from(t).expect("Resolving extensions cannot change kind from ListType(RuntimeType)");
    Ok(())
}
...
pub(super) fn resolve_type_exts(
    ...
    *typ = tm.try_into().expect("Resolving extensions cannot change kind from RuntimeType");
    r
}
