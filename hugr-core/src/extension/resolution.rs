...
use types_mut::{resolve_custom_type_exts, resolve_term_exts, resolve_value_exts};
...
use crate::types::{CustomType, Signature, Term, TypeArg, TypeName};

/// Update all weak Extension pointers inside a [Term].
pub fn resolve_term_extensions(
    typ: &mut Term,
    extensions: &WeakExtensionRegistry,
) -> Result<(), ExtensionResolutionError> {
    let mut used_extensions = WeakExtensionRegistry::default();
    resolve_term_exts(None, typ, extensions, &mut used_extensions)
}
