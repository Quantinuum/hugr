...
impl Term {
    ...
    pub(crate) fn substitute(&self, t: &Substitution) -> Self {
        match self {
            ...
            Term::TupleKind(item_types) => Term::new_list_type(item_types.substitute(t)),
...
pub fn check_term_kind(term: &Term, type_: &Term) -> Result<(), TermTypeError> {
    match (term, type_) {
        ...
        (Term::ListConcat(lists), Term::ListKind(_)) => lists
            .iter()
            .try_for_each(|list| check_term_kind(list, type_)), // ALAN this used the element type, which seems very wrong
